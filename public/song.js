function favfunction(){
    document.getElementById("delfav").className = "fa fa-heartbeat";
...
function addsong(){
    document.getElementById("playlist-container-wrapper").style.display = "block";
    window.addEventListener("click", function(event) {
        if(!event.target.matches('#playlist-container') && !event.target.matches('#member-listbtn2')){
            document.getElementById("playlist-container-wrapper").style.display = "none";
        }
    })
}

function addthistoplaylist(songid,pid){
    document.getElementById('songinp').value = songid;
    document.getElementById('playlistinp').value = pid;
